  ...
  try {
      ...
      for (let i = 0; i < sortedImages.length; i++) {
        ...
        try {
          ...
          if (imageBuffer && imageBuffer.buffer) {
            ...
            // Изображения должны начинаться с E2 и идти последовательно
            // Первое изображение → E2, второе → E3, и т.д.
            const targetRow = 2 + i // Начинаем с строки 2 (после заголовка)
    ...
    for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
      ...
      try {
        ...
        // ИСПРАВЛЕНИЕ: изображения начинаются с E2, а товары с строки 2
        // Товар в строке 2 → изображение в E2, товар в строке 3 → изображение в E3, и т.д.
